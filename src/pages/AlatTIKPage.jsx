...
<div className="page-head"><div><h1>Data Alat TIK</h1><p>Input alat berserial per unit atau stok massal langsung berdasarkan jumlah.</p></div><div className="head-actions"><button className="btn" onClick={() => { const result = handleExport(filtered, 'data-alat-tik.csv'); if (result.success) success(result.message); else error(result.message) }}>Export</button><button className="btn btn-primary" onClick={openCreate}>+ Tambah Data</button></div></div>
    ...
    <div className="card"><div className="toolbar"><SearchBox placeholder="Cari nama, merk, nomor seri, atau lokasi..." value={searchTerm} onChange={event => setSearchTerm(event.target.value)} style={{ flex: 1, minWidth: 220 }} /><Select className="filter-btn" value={filterKategori} onChange={event => setFilterKategori(event.target.value)} placeholder="Semua Jenis" options={kategori} /><Select className="filter-btn" value={filterKondisi} onChange={event => setFilterKondisi(event.target.value)} placeholder="Semua Kondisi" options={kondisiOptions} /><Select className="filter-btn" value={filterLokasi} onChange={event => setFilterLokasi(event.target.value)} placeholder="Semua Lokasi" options={satwil} /></div>{loading ? <LoadingSpinner text="Memuat data..." /> : <><Table columns={columns} data={paginatedData} emptyMessage="Tidak ada data" id="tabel-alat" actions={item => <div className="row-actions"><IconButton className="btn-icon" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.1 2.1 0 0 1 3 3L12 15l-4 1 1-4z"/></svg>} onClick={() => openEdit(item)} /><IconButton className="btn-icon" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>} onClick={() => setDeleteTarget(item)} /></div>} /><Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} itemsPerPage={10} totalItems={filtered.length} /></>}</div>
